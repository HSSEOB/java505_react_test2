...
class App2 extends React.Component{
    ...
    // state의 값이 변경되면 리액트는 화면을 다시 렌더링을 함
    // state의 값을 직접적으로 변경하는 것은 불가능함. setState() 함수를 사용하여 값을 수정
    state = {
        count: 0,
    };
    ...
    //componentDidMount,componentDidUpdate, componentWillUnmout는 리액트의 생명주기에 관련되어 있는 이벤트 함수
    // componentDidMount(): 해당 컴포넌트가 DOM에 추가된 후 동작하느 이벤트
    // componentDidUpdate(): 해당 컴포넌트의 데이터가 변경된 후 동작하는 이벤트
    // componentWillUnmount(): 해당 컴포넌트가 더이상 사용되지 않아 삭제된 후 동작하는 이벤트
    //클래스 컴포넌트에서만 사용 할 수 있음
    //함수 컴포넌트에서는 해당 이벤트 함수들을 사용할 수 없다
